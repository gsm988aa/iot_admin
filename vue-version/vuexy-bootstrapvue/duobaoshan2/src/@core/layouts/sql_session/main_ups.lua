    ...
    while true do
        ...
        if message  == true  then
            ...
            elseif titlex == 51488 then -- 车入过程
                ...
                adc.open(4)
                -- 如果试验位置和试验位置2不等
                if val_gongzuo ~= val_gongzuo2 or val_shiyan ~= val_shiyan2  then
                    sys.wait(100)
                    if wlan_connected == 1 then
                        socket.send(sock, "wrong_state" .. val_shiyan2 .. val_gongzuo2)
                    end
                else
                    -- 第二次状态已经可以和第一次一致，直接运行
                    sys.wait(10)
                    -- 获取试验位置为1 且 断路器合闸为1
                    if val_shiyan2 == 0  and val_gongzuo2 == 1   then
                        -- yyyyyyyyyy
                        log.info("开始车入")
