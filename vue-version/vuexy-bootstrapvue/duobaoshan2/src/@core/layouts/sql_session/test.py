...
serial = serial.Serial('/dev/pts/1', 115200, timeout = 100)   # 打开/dev/ttyS0并设置波特率为115200, 只适用于Linux
print(serial)
...
try:
    ...
    while True:
        ...
        if count >= 40:
            ...
            tailer = load_data[37:40]
            if len(load_data) ==40 and header == 'test' and tailer == 'fin':
                print(load_data)
                pv = int(load_data[4])
                pu1 = load_data[5:10]
                pu2 = load_data[10:15]
