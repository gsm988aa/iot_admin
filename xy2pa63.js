...
// Remove the "0x" prefix from the input string using the slice() method and assign it to the suffix variable
var prefix2  = prefix.slice(2); 

 
// Convert the string to a Buffer object using the Buffer.from() method and assign it to the buffer variable
let buffer2 = Buffer.from(prefix2, 'hex');

// Convert the buffer to a string of hexadecimal values using the toString() method and assign it to the hex variable
let hex = buffer.toString('hex');

// Assign the hex value to the prefix variable with a "0x" prefix
 let prefix3 = '0x' + hex;
  

// Use the 'crc' library to calculate the CRC16 Modbus value of strr and assign it to crc16m
  console.log(prefix3.toString('hex'));

// Use the typeof operator to determine the data type of the prefix variable and log it to the console
  console.log(typeof prefix3);
  
// 计算prefix的MODBUS－RTU CRC-16的校验值给crc16m
    
  const crc16m = crc.crc16modbus(prefix3);
  console.log(crc16m);

// Use the Buffer.concat() method to concatenate the prefix buffer and the crc16m buffer and assign it to bufferx
const bufferx = Buffer.concat([buffer2, Buffer.from(crc16m.toString(16), 'hex')]);

// Convert the bufferx to a string of hexadecimal values using the toString() method and assign it to the buffer4 variable
buffer4 = bufferx.toString('hex');

// Convert the bufferx to a string of ASCII characters using the toString() method and assign it to the buffer variable
buffer = bufferx.toString();
    
  serialport2.write(buffer4,'hex');
